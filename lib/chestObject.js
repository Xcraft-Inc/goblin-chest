...
class ChestObjectLogic extends Elf.Archetype {
  static db = 'chest';
  static indices = ['name', 'generation'];
  ...
  upsert(size, mime, charset, cipher, compress, key, generation, alias) {
    const {state} = this;
    state.size = size;
    state.mime = mime;
    ...
    if (alias && state.name !== alias && !state.alias.includes(alias)) {
      state.alias.push(alias);
    }
...
class ChestObject extends Elf {
   ...
   * @param {*} [alias] alias (for the name)
   */
  async upsert(size, mime, charset, cipher, compress, key, alias) {
