...
class EncryptionShape {
  cipher = enumeration('rsa-with-aes-256-cbc');
  key = string;
}
...
class ChestObjectLogic extends Elf.Archetype {
  ...
  upsert(size, mime, charset, cipher, key) {
    const {state} = this;
    state.size = size;
    state.mime = mime;
    state.charset = charset;
    if (cipher && key) {
      state.encryption = {cipher, key};
    }
...
class ChestObject extends Elf {
   ...
   * @param {*} [key] enryption key
   */
  async upsert(size, mime, charset, cipher, key) {
    this.logic.upsert(size, mime, charset, cipher, key);
    await this.persist();
  }
