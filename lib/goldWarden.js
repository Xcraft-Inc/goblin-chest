...
class GoldWarden extends Elf.Alone {
  ...
  async init(options) {
    if (options?.goldPath) {
      this._goldPath = options?.goldPath;
    }
    await this._reload();
  }
  ...
  async _reload() {
    const {projectPath, appConfigPath} = require('xcraft-core-host');
    ...
    if (!this._goldPath && gold.gitRemote && Git.available) {
      const repository = path.join(appConfigPath, 'var/share');
      const git = new Git(repository);
      if (fse.existsSync(repository)) {
        await git.checkout('master');
        await git.pull();
      } else {
        await git.clone(gold.gitRemote);
      }
      this._goldPath = repository;
    }
