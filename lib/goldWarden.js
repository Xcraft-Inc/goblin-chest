...
const gitBranch = () => {
  ...
  const branch = appVersion.split('.').slice(0, 2).join('.');
  ...
  async _reload(goldPath = null, gitRemote = null) {
    ...
    if (!this._goldPath && this._gitRemote && Git.available) {
      ...
      if (await fse.pathExists(path.join(this._goldPath, '.git'))) {
        ...
        if (remote !== this._gitRemote) {
          await fse.remove(this._goldPath);
        } else {
          try {
            const branch = gitBranch();
            await git.fetch();
            await git.reset(branch); /* Use a clean working dir */
          } catch (ex) {
            this.log.warn(
              `'git reset' has failed, continue anyway: ${ex.message || ex}`
            );
          }
        }
      }
      ...
      if (time) {
        ...
        await chronomancer.restart('goldWardenGit');
      }
      try {
        await this._gitSync();
      } catch (ex) {
    ...
    if (!this._goldPath || !(await fse.pathExists(this._goldPath))) {
      await chronomancer.remove('goldWardenGit');
