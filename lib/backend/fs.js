...
class SHFS {
  ...
  constructor(config) {
    ...
    this.#corrupted = path.join(this.#root, 'corrupted');
    ...
    fse.ensureDirSync(this.#corrupted);
  ...
  async exists(hash) {
    const location = this.location(hash);
    const exists = await fse.pathExists(location);
    if (!exists) {
      return false;
    }

    return true; // TODO: review for performances

    /* Check for validity */
    const _hash = await this.hash(location);
    if (hash === _hash) {
      return true;
    }

    /* The file is corrupted */
    await fse.move(location, path.join(this.#corrupted, hash), {
      overwrite: true,
    });
    return false;
  }
