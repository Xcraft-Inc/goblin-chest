...
class SHFS {
  ...
  async #encrypt(inputFile, outputStream, cert) {
    ...
    await pipeline(inputStream, gzip, encrypter, outputStream);
  ...
  #decrypt(inputFile, encryption, key) {
    ...
    const aesKey = Buffer.alloc(aesKeyIV.length - 16);
    const iv = Buffer.alloc(16);
    aesKeyIV.copy(aesKey, 0, 0, aesKeyIV.length - 16);
    aesKeyIV.copy(iv, 0, aesKeyIV.length - 16);
    const gunzip = zlib.createGunzip();

    const inputStream = fse.createReadStream(inputFile);
    const decrypter = crypto.createDecipheriv(encryption.cipher, aesKey, iv);

    return inputStream.pipe(decrypter).pipe(gunzip);
  }
