...
class SHFS {
  ...
  #decrypt(inputFile, encryptedKey, key) {
    ...
    const inputStream = fse.createReadStream(inputFile);
    const decrypter = crypto.createDecipheriv(this.#cipher, aesKey, iv);

    return inputStream.pipe(decrypter);
  }
  ...
  get(hash, encryption, key) {
    ...
    return this.#decrypt(inputFile, encryption.key, key);
  }
