...
class Chest extends Elf.Alone {
  ...
  async supply(xcraftStream, fileName, streamId, chestObjectId, cert) {
    ...
    try {
      ...
      await object.upsert(size, mime, charset, cipher, compress, key, name);
      return id;
    } catch (ex) {
      this._backend.del(hash);
      throw ex;
  ...
  /**
   * Retrieve chestObjectIds from an alias
   *
   ...
   */
  async getObjectIdsHistoryFromAlias(alias, limit = 10) {
    if (limit < 1) {
      throw new Error(
    ...
    return objects
      ...
      .where((object, $) =>
        $.or(
          object.get('name').eq(alias),
          object.get('alias').some((al) => al.eq(alias))
        )
      )
      .orderBy((object, $) => $.desc(object.get('generation')))
      .limit(limit)
  ...
  /**
   * Retrieve chestObjectIds from a namespace with the whole history
   * @param {string} namespace
   * @returns {Promise<*>}
   */
  async getObjectIdsFromNamespace(namespace) {
    const objects = await this.cryo.reader(ChestObjectLogic.db);
    const list = objects
      .queryArchetype('chestObject', ChestObjectShape)
      ...
      .where((object, $) =>
        $.or(
          object.get('name').like(namespace + '.%'),
          object.get('alias').some((alias) => alias.like(namespace + '.%'))
        )
      ) /* bigger generations first */
      .orderBy((object, $) => $.desc(object.get('generation')))
      .all();

    /* group by name and push id in order (bigger generation first) */
    return list
      .filter(({name}) => name.startsWith(namespace))
      .reduce((names, {id, name, alias}) => {
        if (!names[name]) {
          names[name] = [];
        } else if (!names[name].includes(id)) {
          names[name].push(id);
        }
        alias
          .filter((name) => name.startsWith(namespace))
