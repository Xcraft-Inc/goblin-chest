...
class Chest extends Elf.Alone {
  ...
  async init() {
    ...
    if (!this._isClient) {
      ...
      running = await chronomancer.running('chest');
  ...
  async supply(xcraftStream, fileName, streamId, chestObjectId, cert) {
    ...
    const {hash, size, key, cipher} = await this._backend.put(streamFS, cert);
    ...
    try {
      ...
      await object.upsert(size, mime, charset, cipher, key);
      return id;
    } catch (ex) {
      this._backend.del(hash);
      throw ex;
