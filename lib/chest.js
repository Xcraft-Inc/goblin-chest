...
class Chest extends Elf.Alone {
  ...
  async supply(
    ...
    try {
      ...
      if (namespace) {
        await object.setAlias(namespace, name);
      }
  ...
  async trashAlias(namespace, chestObjectId) {
    const feedId = Elf.createFeed();
    this.quest.defer(async () => await this.killFeed(feedId));

    const id = `chestAlias@${namespace}@${chestObjectId}`;
    const alias = await new ChestAlias(this).create(id, feedId);
    await alias.trash();
  }
  ...
  /**
   * Retrieve the list of chestObject according to a namespace
   *
   ...
   */
  async getObjectIdsFromNamespace(namespace, depth = 1) {
    const reader = await this.cryo.reader(ChestObjectLogic.db);
    ...
    /* Group chestObject id by name */
    const groups = alias.reduce((agg, {id, name}) => {
      const ids = id.split('@');
      const objectId = ids.splice(-2).join('@');
      if (!agg[name]) {
        agg[name] = [];
      }
      agg[name].push(objectId);
      return agg;
    }, {});

    /* Retrieve the latest chestObject of each group */
    for (const group of Object.entries(groups)) {
      const [name, objectIds] = group;
      if (objectIds.length < 2) {
        groups[name] = [objectIds[0]];
        continue;
      }
      groups[name] = reader
        .queryArchetype('chestObject', ChestObjectShape)
        .field('id')
        .where((object) => object.get('id').in(objectIds))
        .orderBy((object, $) => $.desc(object.get('generation')))
        .limit(depth)
        .all();
    }
