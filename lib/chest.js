...
class ChestOptions {
  ...
  goldWarden = option(boolean);
  goldPath = option(string);
  gitRemote = option(string);
}
...
class Chest extends Elf.Alone {
  ...
  async init(options) {
    ...
    if (this._isClient) {
      this.quest.sub(`*::chest.missing-file-needed`, async (_, {msg}) => {
        ...
        const exists = await this._backend.exists(hash);
        if (!exists) {
          return; /* not ours */
        }

        /* In the case where the server has lost a lot of files, provide files
         * one by one in order to prevent ddos.
         */
        const checkMissingLock = locks.getMutex;
        await checkMissingLock.lock(`chest-missing-supply`);
        try {
          ...
          checkMissingLock.unlock(`chest-missing-supply`);
        }
      });
    }
  ...
  async setReplica(enable) {
    ...
    if (time) {
      ...
      await chronomancer.restart('chest', true);
    }

    ({time} = chestConfig.chronomancer.collect);
    if (time) {
      /* Check for orphans only for the server, disabled for local only client */
      await chronomancer.upsert('chestCollect', time, 'chest._collect', {
        orphans: !this._isClient,
      });
   ...
   */
  async _collect(orphans = false) {
    if (!this._backend) {
      throw new Error(`The Chest is not initialized`);
    }

    if (orphans) {
      /* Search and collect orphans (published object never referenced) */
      await this._collectOrphans();
    }

    /* Clean the storage by removing unlinked and trashed objects */
  ...
  async _collectOrphans() {
     ...
     * Extract chestObject@ from raw actions (don't work with JSON,
    ...
    const searchFor = async (db) => {
      ...
      for await (const {refs} of objects) {
        if (!refs) {
          continue;
        }
     ...
     * The IDs are returned by rowid desc order. It means that the
     * first ID is the newer (last updated entry).
     */
    const objectIds = await this.cryo.getIds(
      ChestObjectLogic.db,
      'chestObject',
      {last: true}
    );
    for (const objectId of objectIds) {
      if (!references.has(objectId)) {
        orphans.add(objectId);
      }
    }

    if (!orphans.size) {
      this.log.dbg(`No orphans detected, nothing to collect`);
      return; /* All ChestObjects are referenced somewhere */
    }

    const reader = await this.cryo.reader(ChestObjectLogic.db);
    for (const objectId of new Set(orphans)) {
      const aliasId = reader
        ...
        .get();
      if (aliasId) {
        /* At least one ChestAlias ref, it's not an orphan then continue */
        orphans.delete(objectId);
      }
    }

    const feedId = await this.newQuestFeed();

    const {maxSize} = chestConfig.collect.orphans;
    let totalSize = 0;
    for (const objectId of orphans) {
      if (maxSize > 0) {
        /* Aggregate size until the max size */
        const {size} = await this.cryo.pickAction(
          ChestObjectLogic.db,
          objectId,
          ['size']
        );
      ...
      this.log.dbg(`Trash orphan ${objectId}`);
  ...
  /**
   * Test if this file exists with the provided chestObjectId
   *
   * The chestObjectId is used to compare its hash with the hash
   * provided by filePath. A file exists only if both hash are the same.
   * @param {*} chestObjectId
   * @param {*} filePath
   * @returns {Promise<boolean>}
   */
  async exists(chestObjectId, filePath) {
    if (!this._backend) {
      throw new Error(`The Chest is not initialized`);
    }

    const hash = await this._backend.hash(filePath);
    const exists = await this._backend.exists(hash);
    if (!exists) {
      return false;
    }

    const objectHash = chestObjectId.split('@')[1];
   ...
   *
   * Note that when a fileName is provided, the string is sanitized in order to
   * be compliant with most common file systems. The fileName cannot be changed
   * by supplying again. If you need other file names you must use aliases.
   *
   * It returns the ChestAlias id if a namespace and alias are specified.
   * @param {*} xcraftStream File system path or stream
   * @param {string|null} [fileName] File's name (it's sanitized)
  ...
  async supply(
    ...
    try {
      ...
      await this._backend.onError(streamFS);
      throw ex;
    }
   ...
   * @param {`chestObject@${string}`} chestObjectId ChestObject id
   ...
   * @param {`chestObject@${string}`} chestObjectId
   * @returns {Promise<object>} The location
   */
  async locationTry(chestObjectId) {
    ...
    const hash = chestObjectId.split('@')[1];
    if (await this._backend.exists(hash)) {
      return this._backend.location(hash);
    }
    ...
    if (newHash !== hash) {
      await this._backend.del(newHash);
  ...
  async setVectors(chestObjectId, vectors) {
    const feedId = await this.newQuestFeed();

    const object = await new ChestObject(this).create(chestObjectId, feedId);
    await object.setVectors(vectors);
  }
...
module.exports = {Chest, ChestLogic, ChestOptions};
